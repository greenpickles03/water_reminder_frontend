...
import 'package:water_reminder_front_end/classes/dto/AccountResponseDTO.dart';
...
class ApiService {
  ...
  Future<AccountResponse?> createAccount(AccountRequest accountRequest) async {
    ...
    try {
      final response = await http.post(
        ...
        body: jsonEncode(accountRequest.toJson()),
      );
      print("response: ${response.statusCode}");
      if (response.statusCode == 200) {
        final jsonBody = jsonDecode(response.body);
        print(jsonBody);
        return AccountResponse.fromJson(jsonBody);
      } else {
